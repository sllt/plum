...
func read_list(rdr Reader, start string, end string) (PlumType, error) {
	token := rdr.next()
	if token == nil {
...
func read_vector(rdr Reader) (PlumType, error) {
	lst, e := read_list(rdr, "[", "]")
	if e != nil {
		return nil, e
	}
...
func read_form(rdr Reader) (PlumType, error) {
	...
	case `'`:
		...
		return List{[]PlumType{Symbol{"quote"}, form}, nil}, nil
	case "`":
		rdr.next()
		form, e := read_form(rdr)
		if e != nil {
		...
		return List{[]PlumType{Symbol{"quasiquote"}, form}, nil}, nil
	case `~`:
		rdr.next()
		form, e := read_form(rdr)
		if e != nil {
		...
		return List{[]PlumType{Symbol{"unquote"}, form}, nil}, nil
	case `~@`:
		rdr.next()
		form, e := read_form(rdr)
		if e != nil {
		...
		return List{[]PlumType{Symbol{"splice-unquote"}, form}, nil}, nil
	case `^`:
		rdr.next()
		...
		return List{[]PlumType{Symbol{"with-meta"}, form, meta}, nil}, nil
	case `@`:
		rdr.next()
		form, e := read_form(rdr)
		if e != nil {
		...
		return List{[]PlumType{Symbol{"deref"}, form}, nil}, nil
...
func Read_str(str string) (PlumType, error) {
